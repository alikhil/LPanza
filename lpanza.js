...
function createRoom(){
    ...
    roomsTimers[room] = setTimeout(roomTick, 1, room);
...
function roomTick(room){
   roomsData[room].nextTick = Date.now();
    ...
    roomsData[room].nextTick += 1000 / consts.framesPerSecond;
    var timeToSleep = roomsData[room].nextTick - Date.now ();
    if (timeToSleep < 0) {
        timeToSleep = 0;
    }
